...
4. Un payload de mensaje sin `provider_message_id` estable es rechazado antes de insertar/actualizar `lead_messages`, resolver matching, clasificar, converger acciones o cambiar estado del lead. El boundary conserva el contrato de webhook sin afirmar procesamiento y delega la emisión/auditoría canónica de `inbound_message_rejected` a la capability dueña de Epic 2/inbound; E4-S6 no habilita ni emite ese evento y `REGISTERED_DISABLED` se conserva conforme a E4-S5a.
...
6. El boundary delega matching por teléfono, clasificación, convergencia de `Responder al cliente`, la auditoría canónica del rechazo y cualquier actualización de acciones a los contratos dueños de Epic 2/Epic 1. No reimplementa esas políticas ni apropia su emisión de eventos en el handler.
...
- [ ] Añadir rechazo seguro sin mutación para IDs faltantes y dejar la delegación de la evidencia canónica `inbound_message_rejected` a Epic 2/inbound.
...
- E4-S6 no construye ni persiste `inbound_message_rejected`. El boundary rechaza sin mutar y delega la auditoría a Epic 2/inbound; esa story debe derivar el fingerprint seguro conforme a AD-10 al habilitar el evento. El raw request body nunca se guarda, reserializa ni expone.

### Testing Requirements

- Token ausente/inválido, JSON inválido, `MESSAGES_UPSERT`, `MESSAGES_SET`, `MESSAGES_UPDATE`, `SEND_MESSAGE`, inbound/outbound, `remoteJidAlt`, grupos, broadcast, ID faltante sin emisión de `inbound_message_rejected` por E4-S6, duplicate composite key y estados fuera de orden.
...
- No decidir ni implementar allowlist/clasificación, matching múltiple, convergencia o acciones de respuesta. No habilitar ni emitir `inbound_message_rejected`; su ownership canónico permanece en Epic 2/inbound y E4-S5a permanece `REGISTERED_DISABLED` hasta esa story.
