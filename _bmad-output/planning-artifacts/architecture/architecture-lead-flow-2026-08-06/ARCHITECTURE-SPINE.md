...
scope: Brownfield evolution of the one-advisor Android-first PWA pilot, including follow-up, WhatsApp, Web Push, instrumentation and manual purchase milestones; corporate synchronization is governed as deferred.
status: final
created: '2026-08-06'
updated: '2026-08-06'
binds:
...
- **Binds:** FR-001–FR-013, FR-034–FR-037, NFR-001–NFR-002
...
- **Binds:** FR-009, FR-017, FR-020, FR-032, NFR-007, NFR-013–NFR-014, SM-001–SM-009
...
| --- | --- | --- | --- | --- |
| `next_action_done`, `next_action_postponed`, `next_action_ignored`, `response_action_upserted`, `push_action_taken` | `FOLLOW_UP_ACTION` | `lead_follow_up_actions.id` | resulting `action_version` | versioned action/inbound/capability transition |
...
### AD-11 — [ADOPTED] Manual, non-inferred purchase milestones

- **Binds:** FR-034–FR-037, SM-008
...
- **Rule:** `lead_milestones` records advisor commands such as `PURCHASE_DECISION`, `BLOCKER` and `DELIVERY`. `PURCHASE_DECISION` and `DELIVERY` are each idempotent and unique per lead; `BLOCKER` is repeatable. Their timestamps are operational recording times. No webhook, message classifier or score may create them. FR-037 purchase-to-delivery duration uses the unique persisted `PURCHASE_DECISION` and `DELIVERY` timestamps only.
...
- **Binds:** FR-022–FR-024, NFR-001–NFR-002, NFR-005–NFR-007
...
| --- | --- | --- |
| FR-001–FR-002: capture and manual next action | `app/nuevo`, `lib/leads`, `lead_follow_up_actions` | AD-1, AD-2, AD-4, time convention |
| FR-003–FR-013: customer response and action lifecycle | `lib/leads`, Evolution webhook, action RPCs | AD-4–AD-6 |
...
| FR-034–FR-037: purchase and later milestones | `lib/leads`, `lead_milestones` | AD-4, AD-10, AD-11 |
...
| Partial first-contact repair | `ACCEPTED` items never resend; a `PARTIAL` operation has no automatic or generic retry command. | After the advisor decides whether a manual repair command is allowed, keyed by operation, resource kind and resource version, only for missing/failed items. |
