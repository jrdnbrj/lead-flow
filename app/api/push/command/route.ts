...
export async function GET(request: Request) {
  ...
  if (auth.status !== "AUTHORIZED") return NextResponse.redirect(new URL("/login?next=/dashboard", request.url));
  ...
  if (!deliveryId || !command || !commands.has(command) || !Number.isInteger(actionVersion)) return NextResponse.redirect(new URL("/dashboard?push=invalid", request.url));
  const supabase = await createSupabaseServerClient();
  if (!supabase) return NextResponse.redirect(new URL("/dashboard?push=unavailable", request.url));
  ...
  if (!delivery) return NextResponse.redirect(new URL("/dashboard?push=stale", request.url));
  ...
  return NextResponse.redirect(new URL(`/dashboard?push=${error ? "error" : "applied"}`, request.url));
}
