...
select is((select advisor_user_id from public.leadflow_installation), '1331ad98-0430-4025-88d6-9c1f68083f68'::uuid, 'installation uses only the approved advisor id');
select ok((select not exists (select 1 from public.leadflow_settings where user_id is distinct from '1331ad98-0430-4025-88d6-9c1f68083f68'::uuid)), 'any existing settings rows remain attached to the approved owner');

select throws_ok(
  $$insert into public.leadflow_installation (singleton, advisor_user_id) values (true, '1331ad98-0430-4025-88d6-9c1f68083f68')$$,
  '23505',
  null,
  'duplicate singleton is rejected'
);
select throws_ok(
  $$insert into public.leadflow_installation (singleton, advisor_user_id) values (false, '1331ad98-0430-4025-88d6-9c1f68083f68')$$,
  '23514',
  null,
...
select lives_ok(
  $$select public.e4_s2_apply_installation_migration_fixture('1331ad98-0430-4025-88d6-9c1f68083f68'::uuid)$$,
...
select throws_ok(
  $case$do $$
    begin
      ...
      perform public.e4_s2_apply_installation_migration_fixture('1331ad98-0430-4025-88d6-9c1f68083f68'::uuid);
    end
  $$;$case$,
