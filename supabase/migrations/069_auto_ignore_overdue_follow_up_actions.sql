-- Automatically close reminders that have been overdue for more than 15 days.
-- The action remains the business authority; Push and WhatsApp projections are
-- canceled as part of the same server-side maintenance operation.
...
begin
  ...
  set status = 'IGNORED',
      completed_at = p_now,
      note = case
        when note is null or btrim(note) = '' then 'Ignorada automáticamente por vencer hace más de 15 días.'
        else note || ' · Ignorada automáticamente por vencer hace más de 15 días.'
      end,
      updated_at = p_now
  where status in ('PENDING', 'POSTPONED')
    and scheduled_for < p_now - interval '15 days';
...
is 'Closes pending follow-up actions more than 15 days overdue and cancels unaccepted Push/WhatsApp projections.';
