-- E3-S5/S7: fenced effect attempts and durable per-resource outcomes.

update public.leadflow_event_registry
set emit_status = 'ENABLED', updated_at = now()
where event_type in ('external_effect_claimed', 'external_effect_io_started', 'external_effect_result_recorded', 'first_contact_result');
...
declare owner_id uuid; effect_row public.external_effects; item_row public.lead_contact_operation_items; operation_row public.lead_contact_operations; attempt_no integer; attempt_row public.external_effect_attempts; event_result jsonb;
begin
  ...
  insert into public.external_effect_attempts(effect_id, attempt_no, claim_token_digest) values (effect_row.id, attempt_no, p_claim_token_digest) returning * into attempt_row;
...
create or replace function public.begin_first_contact_effect_io_v1(p_effect_id uuid, p_attempt_no integer, p_claim_token_digest text)
returns jsonb language plpgsql security definer set search_path = public, auth, extensions as $$
...
begin
  ...
  if p_result_kind = 'ACCEPTED' then
    insert into public.lead_messages(lead_id, direction, status, provider_message_id, phone, body, created_at) values (lead_row.id, 'OUTBOUND', 'SENT', p_provider_message_id, lead_row.phone, p_message_body, now()) returning id into message_id;
  end if;
  ...
  event_result := public.append_leadflow_event_v1(jsonb_build_object('event_type','external_effect_result_recorded','schema_version',1,'occurred_at',now(),'source','PWA','stage','FIRST_CONTACT','actor_kind','ADVISOR','actor_id',owner_id,'correlation_id',gen_random_uuid(),'idempotency_key',p_claim_token_digest || ':external-result:' || p_result_kind,'result','APPLIED','aggregate_type','EXTERNAL_EFFECT','aggregate_id',effect_row.id,'aggregate_version',effect_row.effect_version,'payload','{}'::jsonb,'identity_components',jsonb_build_array(jsonb_build_object('name','aggregate_type','value','EXTERNAL_EFFECT'),jsonb_build_object('name','aggregate_id','value',effect_row.id),jsonb_build_object('name','aggregate_version','value',effect_row.effect_version))));
...
revoke all on function public.claim_first_contact_effect_v1(uuid,text) from public, anon;
revoke all on function public.begin_first_contact_effect_io_v1(uuid,integer,text) from public, anon;
revoke all on function public.record_first_contact_effect_result_v1(uuid,integer,text,text,text,text,text) from public, anon;
grant execute on function public.request_first_contact_v1(uuid,text,jsonb,text) to authenticated;
grant execute on function public.claim_first_contact_effect_v1(uuid,text) to authenticated;
grant execute on function public.begin_first_contact_effect_io_v1(uuid,integer,text) to authenticated;
grant execute on function public.record_first_contact_effect_result_v1(uuid,integer,text,text,text,text,text) to authenticated;
