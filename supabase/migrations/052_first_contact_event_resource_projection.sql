-- E3: project multi-vehicle resource kinds uniquely in the event payload.
-- The operation still keeps one item/effect per model and resource; the event
-- contract only describes the resource categories involved.
  ...
  event_result := public.append_leadflow_event_v1(
    jsonb_build_object(
      ...
      'payload', jsonb_build_object(
        'lead_id', p_lead_id,
        'requested_resources', (
          select jsonb_agg(requested_resource_kinds.resource_kind order by requested_resource_kinds.resource_order)
          from (
            select distinct
              value->>'resource_kind' as resource_kind,
              case value->>'resource_kind'
                when 'MESSAGE' then 1
                when 'PHOTOS' then 2
                when 'TECHNICAL_SHEET' then 3
              end as resource_order
            from jsonb_array_elements(p_items)
          ) as requested_resource_kinds
        ),
