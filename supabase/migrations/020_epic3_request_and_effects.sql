-- E3-S4/S5/S7: request/materialize first contact and append requested/result events.

update public.leadflow_event_registry
set emit_status = 'ENABLED', updated_at = now()
where event_type in ('first_contact_requested', 'first_contact_result');
...
begin
  ...
  for item_input in select value from jsonb_array_elements(p_items) loop
    ...
    if availability = 'AVAILABLE' then
      insert into public.external_effects (lead_id, effect_kind, business_key, item_id)
      values (p_lead_id, 'WHATSAPP_FIRST_CONTACT', operation_row.id::text || ':' || item_key || ':' || resource_version, item_row.id)
