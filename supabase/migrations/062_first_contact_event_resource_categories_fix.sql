-- Keep the multi-vehicle item projection while emitting unique resource
-- categories for the existing first_contact_requested event contract.
begin;
...
begin
  ...
  event_result := public.append_leadflow_event_v1(jsonb_build_object(
    ...
    'payload', jsonb_build_object(
      'lead_id', p_lead_id,
      'requested_resources', (
        select jsonb_agg(resource_kind order by resource_kind)
        from (
          select distinct value->>'resource_kind' as resource_kind
          from jsonb_array_elements(p_items)
        ) resource_categories
      ),
