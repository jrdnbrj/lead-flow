-- E3: allow an unattempted available resource to be sent independently.
-- The original first-contact operation may stop after MESSAGE fails while
-- PHOTOS/TECHNICAL_SHEET remain READY. This forward-only change lets the
-- existing fenced retry command claim READY as well as FAILED effects.
...
begin
  ...
  elsif effect_row.state not in ('READY', 'FAILED') then
