-- E3 runtime fix: materialize transition aggregate values before event append.
...
declare owner_id uuid; effect_row public.external_effects; item_row public.lead_contact_operation_items; operation_row public.lead_contact_operations; lead_row public.leads; attempt_row public.external_effect_attempts; next_operation_status text; resource_results jsonb; event_result jsonb; message_id uuid; effect_event_id uuid; effect_event_version bigint; effect_provider text; operation_event_id uuid; operation_event_version bigint; operation_lead_id uuid;
begin
  ...
  effect_event_id := effect_row.id; effect_event_version := effect_row.effect_version; effect_provider := effect_row.provider;
  ...
  event_result := public.append_leadflow_event_v1(jsonb_build_object('event_type','external_effect_result_recorded','schema_version',1,'occurred_at',now(),'source','PWA','stage','EXTERNAL_EFFECT','actor_kind','ADVISOR','actor_id',owner_id,'correlation_id',gen_random_uuid(),'idempotency_key',p_claim_token_digest || ':external-result:' || p_result_kind,'result','APPLIED','payload',jsonb_build_object('provider',effect_provider),'identity_components',jsonb_build_array(jsonb_build_object('name','aggregate_type','value','EXTERNAL_EFFECT'),jsonb_build_object('name','aggregate_id','value',effect_event_id),jsonb_build_object('name','aggregate_version','value',effect_event_version)),'aggregate_type','EXTERNAL_EFFECT','aggregate_id',effect_event_id,'aggregate_version',effect_event_version));
  ...
  operation_event_id := operation_row.id; operation_event_version := operation_row.operation_version; operation_lead_id := operation_row.lead_id;
  select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object('resource', i.resource_kind, 'result', i.result, 'provider_message_id', case when i.result='ACCEPTED' then i.provider_message_id else null end)) order by i.item_key), '[]'::jsonb) into resource_results from public.lead_contact_operation_items i where i.operation_id=operation_event_id and i.result is not null;
  event_result := public.append_leadflow_event_v1(jsonb_build_object('event_type','first_contact_result','schema_version',1,'occurred_at',now(),'source','PWA','stage','FIRST_CONTACT','actor_kind','ADVISOR','actor_id',owner_id,'correlation_id',gen_random_uuid(),'idempotency_key',p_claim_token_digest || ':result:' || p_result_kind,'result',null,'aggregate_type','LEAD_CONTACT_OPERATION','aggregate_id',operation_event_id,'aggregate_version',operation_event_version,'payload',jsonb_build_object('lead_id',operation_lead_id,'resource_results',resource_results),'identity_components',jsonb_build_array(jsonb_build_object('name','aggregate_type','value','LEAD_CONTACT_OPERATION'),jsonb_build_object('name','aggregate_id','value',operation_event_id),jsonb_build_object('name','aggregate_version','value',operation_event_version))));
  return jsonb_build_object('status',p_result_kind,'effect_id',effect_event_id,'attempt_no',p_attempt_no,'operation_status',operation_row.status,'operation_version',operation_event_version,'event',event_result);
end;
$$;
