create table if not exists public.lead_inbound_manual_decisions (
  id uuid primary key default gen_random_uuid(),
  idempotency_key text not null unique check (length(btrim(idempotency_key)) between 16 and 200),
  lead_id uuid not null references public.leads(id),
  source_message_id uuid references public.lead_messages(id),
  action_id uuid references public.lead_follow_up_actions(id),
  decision text not null check (decision in ('REQUIRES_RESPONSE', 'NO_RESPONSE_REQUIRED')),
  result jsonb not null,
  created_at timestamptz not null default now()
);

alter table public.lead_inbound_manual_decisions enable row level security;

create policy lead_inbound_manual_decisions_owner_read on public.lead_inbound_manual_decisions
  for select to authenticated
  using (exists (select 1 from public.leads where leads.id = lead_inbound_manual_decisions.lead_id and leads.user_id = auth.uid()));
...
begin
  ...
  select result into existing from public.lead_inbound_manual_decisions where idempotency_key = p_idempotency_key;
  ...
  if p_decision = 'REQUIRES_RESPONSE' then
    ...
    result := public.transition_lead_follow_up_action_v1(action_id, 'IGNORED', p_expected_action_version, null, 'Marcado como no requiere respuesta.', p_idempotency_key || ':action', null);
  end if;
