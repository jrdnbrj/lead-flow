-- Fix the claim fence variable/column name collision discovered in 047.
-- Forward-only replacement; historical migrations remain immutable.
...
begin
  ...
  claim_token_digest_value := encode(digest(gen_random_uuid()::text || delivery_row.id::text, 'sha256'), 'hex');

  insert into public.external_effect_attempts (
    effect_id, attempt_no, claim_token_digest
  ) values (
    delivery_row.effect_id, next_attempt, claim_token_digest_value
  );
  ...
  set status = 'CLAIMED',
      ...
      claim_token_digest = claim_token_digest_value,
      claimed_at = p_now,
      updated_at = p_now
  where id = delivery_row.id
    and status = 'SCHEDULED';
