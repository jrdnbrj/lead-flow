...
function isDueAction(action: FollowUpAction): boolean {
  return isOpenAction(action) && !isLeadReminderTooOld(action.scheduledFor) && isLeadReminderDue(action.scheduledFor);
}

function getNextOpenAction(lead: Lead): FollowUpAction | null {
  return lead.followUpActions.filter((action) => isOpenAction(action) && !isLeadReminderTooOld(action.scheduledFor)).sort((a, b) => new Date(a.scheduledFor).getTime() - new Date(b.scheduledFor).getTime())[0] ?? null;
}
