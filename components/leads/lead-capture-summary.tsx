...
export function LeadCaptureSummary({ lead }: { lead: Pick<Lead, "fullName" | "phone" | "nationalId" | "email" | "carModels" | "timeframe" | "paymentMethods" | "tradeInCar" | "notes" | "score"> }) {
  return <section className="rounded-xl border border-[#dce5ef] bg-[#f8fbff] p-2.5" aria-label="Resumen del lead guardado">
    ...
    <div className="mt-2 grid grid-cols-2 gap-1.5"><SummaryCell label="Interés" value={lead.carModels.join(", ")} icon={<CarFront size={11} />} /><SummaryCell label="Compra" value={getTimeframeLabel(lead.timeframe)} icon={<Target size={11} />} /><SummaryCell label="Pago" value={getPaymentMethodLabels(lead.paymentMethods)} icon={<CreditCard size={11} />} /><SummaryCell label="Parte de pago" value={lead.tradeInCar ? "Sí, tiene vehículo" : "No"} icon={<CarFront size={11} />} />{lead.nationalId ? <SummaryCell label="Cédula" value={lead.nationalId} icon={<FileText size={11} />} /> : null}{lead.email ? <SummaryCell label="Correo" value={lead.email} icon={<FileText size={11} />} /> : null}</div>
