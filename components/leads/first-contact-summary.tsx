...
export function FirstContactSummary({ lead, initialOperation }: { lead: Pick<Lead, "id" | "fullName" | "phone" | "carModels"> & Partial<Pick<Lead, "whatsappStatus" | "lastAgentMessageAt" | "lastMessageDirection">>; initialOperation?: FirstContactOperationResult | null }) {
  ...
  return <section className="mt-2 rounded-xl border border-[#dce5ef] bg-[#f8fbff] p-2" aria-label="Resumen del primer contacto">
    <div className="flex items-center justify-between gap-3"><p className="text-[10px] font-black uppercase tracking-[0.08em] text-[var(--muted)]">Primer contacto</p>{!operation && !recoveryMode ? <button type="button" disabled={isSending} onClick={() => void start()} className="button-primary min-h-9 px-3 py-1.5 text-[11px]">{isSending ? <LoaderCircle size={14} className="animate-spin" /> : <Send size={14} />}<span>{isSending ? "Preparando…" : "Iniciar"}</span></button> : null}</div>
    {recoveryMode ? <><div className="mt-1.5 grid grid-cols-3 gap-1"><RecoveryResource label="Mensaje" status={recoveryMessageStatus} /><RecoveryResource label="Fotos" status="No confirmado" /><RecoveryResource label="Ficha técnica" status="No confirmado" /></div><p className="mt-1.5 text-[10px] leading-4 text-[var(--muted)]">Hay evidencia de un intento, pero falta el detalle del primer contacto. El botón pequeño de la card permite intentar todo de nuevo.</p></> : null}
...
function RecoveryResource({ label, status }: { label: string; status: string }) {
  return <div className="flex min-h-[52px] min-w-0 flex-col items-center justify-center rounded-lg border border-black/[0.06] bg-white p-2 text-center"><p className="text-[9px] font-black leading-3">{label}</p><span className="mt-1 rounded-full bg-[#fff8df] px-1.5 py-0.5 text-[9px] font-black leading-3 text-[#8a5b00]">{status}</span></div>;
}
...
function ResourceResult({ item, retrying, onRetry }: { item: FirstContactOperationResult["items"][number]; retrying: boolean; onRetry: () => void }) {
  ...
  const retryable = item.availability === "AVAILABLE" && (item.result === null || item.result === "FAILED" || item.result === "UNKNOWN");
