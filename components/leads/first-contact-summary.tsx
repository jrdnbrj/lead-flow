...
import { getFirstContactColorOptionsAction, retryFirstContactRecoveryResourceAction, retryFirstContactResourceAction, startFirstContactAction } from "@/lib/leads/actions";
...
export function FirstContactSummary({ lead, initialOperation }: { lead: Pick<Lead, "id" | "fullName" | "phone" | "carModels"> & Partial<Pick<Lead, "whatsappStatus" | "lastAgentMessageAt" | "lastMessageDirection">>; initialOperation?: FirstContactOperationResult | null }) {
  ...
  async function startWithOptionalColorSelection() {
    if (isSending || isPreparingColors || operation) return;
    setIsPreparingColors(true);
    setError(null);
    try {
      const response = await getFirstContactColorOptionsAction(lead.id);
      if (!response.success) {
        setError(response.error || "No pudimos cargar las opciones de color.");
        return;
      }
      ...
      setError("No pudimos preparar las opciones de color. Puedes reintentarlo.");
    } finally {
  ...
  return <section className="mt-2 rounded-xl border border-[#dce5ef] bg-[#f8fbff] p-2" aria-label="Resumen del primer contacto">
    <div className="flex items-center justify-between gap-3"><p className="text-[10px] font-black uppercase tracking-[0.08em] text-[var(--muted)]">Primer contacto</p>{!operation && !recoveryMode ? <button type="button" disabled={isSending || isPreparingColors} aria-busy={isSending || isPreparingColors} onClick={() => void startWithOptionalColorSelection()} className="button-primary min-h-9 px-3 py-1.5 text-[11px]">{isSending || isPreparingColors ? <LoaderCircle size={14} className="animate-spin" /> : <Send size={14} />}<span>{isSending || isPreparingColors ? "Preparando…" : "Iniciar"}</span></button> : null}</div>
    ...
    {isColorSelectorOpen ? <FirstContactColorSelector lead={lead} open initialModels={colorModels} onCancel={() => setIsColorSelectorOpen(false)} onConfirm={(colorSelections) => void start(colorSelections)} /> : null}
  </section>;
}
...
function ResourceResult({ item, leadModels, retrying, onRetry }: { item: FirstContactOperationResult["items"][number]; leadModels: string[]; retrying: boolean; onRetry: () => void }) {
  ...
  const label = firstContactItemLabel(resource, item.itemKey, leadModels, item.selectedColorName, item.resourceSnapshot?.modelName);
...
function firstContactItemLabel(resource: FirstContactResource, itemKey: string, leadModels: string[], selectedColorName?: string | null, snapshotModelName?: string | null): string {
  ...
  const modelName = snapshotModelName ?? leadModels[modelIndex];
