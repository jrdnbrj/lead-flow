...
export function PendingNotifications({ leads }: { leads: Lead[] }) {
  ...
  return <section className="rounded-[28px] border border-[#dce5ef] bg-[#f8fbff] p-4 shadow-[0_8px_24px_rgba(16,24,40,0.04)] sm:p-5" aria-label="Notificaciones pendientes">
    ...
    {pending.length ? <div className="mt-4 space-y-2">{pending.map((row) => { const due = isLeadReminderDue(row.action.scheduledFor); const busy = busyId === row.action.id; return <article key={row.action.id} className="rounded-2xl border border-black/[0.06] bg-white p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="truncate text-sm font-black">{row.lead.fullName}</p><p className="mt-0.5 text-xs font-bold text-[var(--ink)]">{getNextActionLabel(row.action.actionType)} <span className="font-normal text-[var(--muted)]">· versión {row.action.actionVersion ?? 1}</span></p><p className={`mt-1 flex items-center gap-1 text-[11px] font-semibold ${due ? "text-[#b94910]" : "text-[var(--muted)]"}`}><Clock3 size={12} />{due ? "Vencida · " : "Programada · "}{formatNextActionDate(row.action.scheduledFor) ?? row.action.scheduledFor}</p>{row.action.note ? <p className="mt-1 line-clamp-2 text-[11px] text-[var(--muted)]">{row.action.note}</p> : null}</div><div className="flex flex-wrap gap-1.5"><button type="button" disabled={busy} onClick={() => void transition(row, "DONE")} className="inline-flex h-8 items-center gap-1 rounded-lg bg-[#e4f8e9] px-2.5 text-[11px] font-black text-[#18733a] disabled:opacity-50"><Check size={13} />Hecha</button><button type="button" disabled={busy} onClick={() => void transition(row, "IGNORED")} className="inline-flex h-8 items-center gap-1 rounded-lg bg-[#f1f1f1] px-2.5 text-[11px] font-black text-[#777c86] disabled:opacity-50"><Ban size={13} />Ignorar</button></div></div><div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-black/[0.06] pt-2.5"><span className="mr-1 inline-flex items-center gap-1 text-[10px] font-black text-[var(--muted)]"><RotateCcw size={12} />Posponer</span>{shortcuts.map((shortcut) => <button key={shortcut.value} type="button" disabled={busy} onClick={() => void transition(row, "POSTPONED", shortcut.value)} className="rounded-lg bg-[#edf3ff] px-2.5 py-1.5 text-[10px] font-black text-[#3c5f9b] disabled:opacity-50">{shortcut.label}</button>)}</div></article>; })}</div> : <p className="mt-4 rounded-xl bg-white px-3 py-3 text-xs font-semibold text-[var(--muted)]">No tienes notificaciones pendientes.</p>}
