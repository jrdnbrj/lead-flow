...
const oneMinuteTestShortcut = { label: "En 1 min" };
...
export function PendingNotifications({ leads }: { leads: Lead[] }) {
  ...
  return <section className="rounded-2xl border border-[#dce5ef] bg-[#f8fbff] p-2.5 shadow-[0_8px_24px_rgba(16,24,40,0.04)] sm:p-3" aria-label="Notificaciones pendientes">
    <div className="flex items-center gap-2"><span className="grid size-7 place-items-center rounded-lg bg-[#e7effd] text-[#3c5f9b]"><Bell size={15} /></span><h2 className="text-sm font-black">Notificaciones pendientes</h2><span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-black text-[var(--muted)]">{pending.length}</span></div>
    {pending.length ? <div className="mt-2 space-y-1.5">{pending.map((row) => { const due = isLeadReminderDue(row.action.scheduledFor); const busy = busyId === row.action.id; return <article key={row.action.id} aria-busy={busy} className="rounded-xl border border-black/[0.06] bg-white px-2.5 py-2"><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{row.lead.fullName} <span className="font-semibold text-[var(--muted)]">· {row.lead.phone}</span></p><p className={`mt-0.5 flex items-center gap-1 text-[10px] font-bold ${due ? "text-[#b94910]" : "text-[var(--muted)]"}`}><Clock3 size={11} />{getNextActionLabel(row.action.actionType)} · {due ? "Vencida" : formatNextActionDate(row.action.scheduledFor) ?? row.action.scheduledFor}</p>{row.action.note ? <p className="mt-0.5 truncate text-[10px] text-[var(--muted)]">{row.action.note}</p> : null}</div><div className="flex flex-wrap gap-1"><button type="button" disabled={busy} onClick={() => void transition(row, "DONE")} className="inline-flex h-7 items-center gap-1 rounded-lg bg-[#e4f8e9] px-2 text-[10px] font-black text-[#18733a] disabled:opacity-50"><Check size={12} />{busy ? "…" : "Hecha"}</button><button type="button" disabled={busy} onClick={() => void transition(row, "IGNORED")} className="inline-flex h-7 items-center gap-1 rounded-lg bg-[#f1f1f1] px-2 text-[10px] font-black text-[#777c86] disabled:opacity-50"><Ban size={12} />{busy ? "…" : "Ignorar"}</button></div></div>{busy ? <p className="mt-1 text-[10px] font-bold text-[var(--muted)]" aria-live="polite">Actualizando…</p> : null}<div className="mt-1.5 flex flex-wrap items-center gap-1 border-t border-black/[0.06] pt-1.5"><span className="mr-0.5 inline-flex items-center gap-1 text-[10px] font-black text-[var(--muted)]"><RotateCcw size={11} />Posponer</span><button type="button" disabled={busy} onClick={() => void transition(row, "POSTPONED", undefined, new Date(Date.now() + 60_000).toISOString())} className="rounded-lg bg-[#fff4d6] px-2 py-1 text-[10px] font-black text-[#8a5b00] disabled:opacity-50">{busy ? "…" : oneMinuteTestShortcut.label}</button>{shortcuts.map((shortcut) => <button key={shortcut.value} type="button" disabled={busy} onClick={() => void transition(row, "POSTPONED", shortcut.value)} className="rounded-lg bg-[#edf3ff] px-2 py-1 text-[10px] font-black text-[#3c5f9b] disabled:opacity-50">{busy ? "…" : shortcut.label}</button>)}</div></article>; })}</div> : <p className="mt-2 rounded-lg bg-white px-2.5 py-2 text-[11px] font-semibold text-[var(--muted)]">No tienes notificaciones pendientes.</p>}
