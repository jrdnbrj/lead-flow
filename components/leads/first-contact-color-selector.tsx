...
export function FirstContactColorSelector({ lead, open, onCancel, onConfirm }: { lead: ColorSelectorLead; open: boolean; onCancel: () => void; onConfirm: (selections: FirstContactColorSelection[]) => void }) {
  ...
  const preloadModelPhotos = useCallback((model: FirstContactColorModelOption) => {
  ...
  useEffect(() => {
    ...
    const fallbackTimer = window.setTimeout(() => models.forEach(preloadModelPhotos), 900);
    return () => window.clearTimeout(fallbackTimer);
  }, [models, open, preloadModelPhotos]);
  ...
  return <section className="rounded-xl border border-black/[0.07] bg-[#fbfaf7] p-2.5">
    <div className="flex items-center gap-2.5">
      <div aria-label="Vista previa de miniatura" className="relative h-16 w-24 shrink-0 overflow-hidden rounded-lg bg-[#f0eee8]">
        {previewUrl ? <Image loader={directImageLoader} unoptimized priority fetchPriority="high" src={previewUrl} alt={`${model.modelName}${selectedColor ? ` en ${selectedColor.name}` : ""}`} fill sizes="96px" onLoad={onDefaultImageLoad} className="object-cover" /> : <div className="grid size-full place-items-center text-[var(--muted)]"><Palette size={18} /></div>}
      </div>
