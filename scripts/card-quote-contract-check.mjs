...
const leadCapture = fs.readFileSync("components/leads/lead-capture-form.tsx", "utf8");
const workspace = fs.readFileSync("components/quotes/quote-workspace.tsx", "utf8");
for (const required of ["<CardQuoteTool lead={lead} compact />", "CardQuoteTool"]) assert.ok(dashboard.includes(required), `dashboard missing ${required}`);
