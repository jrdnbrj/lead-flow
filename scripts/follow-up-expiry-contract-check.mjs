...
const migration = fs.readFileSync("supabase/migrations/080_auto_ignore_overdue_follow_up_actions_after_five_days.sql", "utf8");
...
for (const token of [
  ...
  "status = 'IGNORED'",
  "scheduled_for < p_now - interval '5 days'",
  "CANCELED_ACTION_EXPIRED",
  "ACTION_EXPIRED",
]) assert.ok(migration.includes(token), `expiry migration missing ${token}`);
for (const source of [followUp, dashboard, pending, actions, push, whatsapp]) assert.match(source, /5 days|5 días|TooOld|expired_follow_up|ACTION_EXPIRED|CANCELED_ACTION_EXPIRED/u, "5-day expiry guard missing");
assert.match(followUp, /reference\.getTime\(\) - scheduledAt > LEAD_REMINDER_MAX_OVERDUE_MS/u);
assert.match(followUp, /const hour = shortcut === "POSTPONE_TOMORROW" \? 9 : 14/u);
assert.match(followUp, /day: local\.day \+ days, hour/u);
